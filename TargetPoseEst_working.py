...
def merge_estimations(target_pose_dict):
    ...
    # Create a dictionary to store merged target poses
    merged_target_poses = {}

    for target_key, target_pose in target_pose_dict.items():
        target_class = target_key.split('_')[0]  # Extract the target class from the key
        if target_class not in merged_target_poses:
            merged_target_poses[target_class] = [target_pose]
        else:
            # Check if the current target pose is close to any previously merged poses
            merged = False
            for merged_pose in merged_target_poses[target_class]:
                distance = np.sqrt((target_pose['x'] - merged_pose['x'])**2 + (target_pose['y'] - merged_pose['y'])**2)
                if distance <= distance_threshold:
                    # Merge the poses by averaging their positions
                    merged_pose['x'] = (merged_pose['x'] + target_pose['x']) / 2
                    merged_pose['y'] = (merged_pose['y'] + target_pose['y']) / 2
                    merged = True
                    break
            if not merged:
                merged_target_poses[target_class].append(target_pose)

    # Flatten the merged poses back into a single dictionary
    for target_class, merged_poses in merged_target_poses.items():
        for i, merged_pose in enumerate(merged_poses):
            target_key = f'{target_class}_{i}'
            target_est[target_key] = merged_pose
    #########
