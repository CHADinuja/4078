...
def estimate_pose(camera_matrix, obj_info, robot_pose):
    ...
    target_dimensions_dict = {'Orange': [7.3*10**-2, 7.3*10**-2, 7*10**-2], 'Lemon': [7.9*10**-2, 5.3*10**-2, 4.1*10**-2], 
                              'Lime': [7.5*10**-2, 5*10**-2, 4.5*10**-2], 'Tomato': [7.5*10**-2, 7.5*10**-2, 6.5*10**-2], 
                              'Capsicum': [8.4*10**-2, 8.5*10**-2, 9.7*10**-2], 'potato': [9.6*10**-2, 7.1*10**-2, 6.3*10**-2], 
                              'Pumpkin': [8.5*10**-2, 8.6*10**-2, 8.8*10**-2], 'Garlic': [6.8*10**-2, 6.3*10**-2, 7.6*10**-2]}
    #########
...
def merge_estimations(target_pose_dict):
    ...
    """
    target_est = {}
    # guaranteed that fruits of the same type are 15cm apart (fruit to fruit not fruit to marker)
    distance_threshold = 0.15
    target_est = target_pose_dict
    print("target_pose_dict.items():",target_pose_dict.items())
    for key, pose in target_pose_dict.items():
        target_class, occurrence = key.split('_')
        target_found = False

        # Ensure that the target class name is in lowercase
        target_class = target_class.lower()

        # Check if there's a target of the same class within the distance threshold
        for merged_key, merged_pose in target_est.items():
            merged_class, merged_occurrence = merged_key.split('_')

            # Ensure that the merged target class name is in lowercase
            merged_class = merged_class.lower()

            if target_class == merged_class and abs(pose['x'] - merged_pose['x']) < distance_threshold and abs(
                    pose['y'] - merged_pose['y']) < distance_threshold:
                # Merge the estimations by averaging the positions
                merged_pose['x'] = (pose['x'] + merged_pose['x']) / 2
                merged_pose['y'] = (pose['y'] + merged_pose['y']) / 2
                target_est[merged_key] = merged_pose
                target_found = True
                break
    ...
    target_est_lowercase = convert_keys_to_lowercase(target_est)


    return target_est_lowercase
...
if __name__ == "__main__":
    ...
    # init YOLO model
    model_path = f'{script_dir}/YOLO/model/yolov8_model_2.pt'
