...
class Operate:
    ...
    # wheel control
    def control(self, motion=None, drive_time=None):
        if not self.fruit_search:       
            ...
            lv, rv = self.pibot.set_velocity(motion, tick = 10,turning_tick=30, time = drive_time)
...
def check_for_target(robot_pose):
    ...
    target_map = TargetPoseEst.estimate_pose(operate.folder, operate.camera_matrix, completed_img_dict, args.runtype)
...
def get_robot_pose(aruco_true_pos):
    ####################################################
    ...
    image = operate.pibot.get_image()
    # cv2.imshow("img", image)
    # cv2.waitKey(0)
    lms, aruco_img = operate.aruco_det.detect_marker_positions(image)
    # call_umeyama(lms, aruco_true_pos) -> still needs work
    if lms == []:
        return operate.ekf.get_state_vector()[0:3,0]
    else:
        for i in range(len(lms)):
            lms[i].position = aruco_true_pos[lms[i].tag-1].reshape(2,1)
        ...
        pose = operate.ekf.get_state_vector()[0:3,0]
        print(pose)
        '''if pose[2] > 2*np.pi:
            pose[2] -= 2*np.pi'''
        operate.ekf.robot.state[2] = (pose[2]+2*np.pi)%(2*np.pi)
...
def drive_to_point(waypoint, robot_pose, aruco_true_pos):
    ...
    while abs(orientation_diff) > rot_threshold:
        ...
        operate.record_data()
        operate.save_image()
        # target_map = check_for_target(operate.ekf.robot.state[0:3])
        # visualise
        operate.draw(canvas)
            ...
            while start:
                ...
                if operate.ekf_on:
                    ...
                    while goal_idx < len(goal_list):
                        ...
                        rrtc = RRTC(start=start, goal=goal, width=2.5, height=2.5, obstacle_list=all_obstacles_wrt_current_goal,
                        ...
                        while path_robot_to_goal == None:
                            ...
                            print('pre:', pre)
                            all_obstacles_wrt_current_goal,move_back_flag = find_obs_wrt_goal(goal, goal_list, obstacle_list, obs_rad, pre,start)
                            ...
                            rrtc = RRTC(start=start, goal=goal, width=2, height=2, obstacle_list=all_obstacles_wrt_current_goal,
                                    expand_dis=0.15, path_resolution=0.01)
                            
                            path_robot_to_goal = rrtc.planning()
                            if path_robot_to_goal == None:
                                
                                obs_rad = obs_rad-0.05
                        ...
                        plot_path(path_robot_to_goal, obs_list,'path',None,goal_idx, apple_list,lemon_list,orange_list,pear_list,strawberry_list)
                        plt.savefig
                        ...
                        while node_idx < len(path_robot_to_goal):
                            ...
                            if distance_to_goal < 0.35:
                                ...
                                plot_path(path_robot_to_goal, obs_list,'path',robot_pose_list,goal_idx,apple_list,lemon_list,orange_list,pear_list,strawberry_list)
                                break
                            node_idx += 1
