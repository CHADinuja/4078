...
def calibrateWheelRadius():
    ...
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        scale += 1/(wheel_vel * delta_time) # TODO: replace with your code to compute the scale parameter using wheel_vel and delta_time
...
def calibrateBaseline(scale):
        ...
        while True:
            ...
            while True:
                ...
                # Spin the robot at the given speed for the given time
                    ppi.set_velocity([0, 1], tick=20,turning_tick=wheel_vel, time = 20)
                    temp_time -= 20
                else:
                    ppi.set_velocity([0, 1], tick=20,turning_tick=wheel_vel, time = temp_time)
                    break
    ...
    baseline = 0
    for delta_time, wheel_vel in zip(delta_times, wheel_velocities_range):
        baseline += scale*wheel_vel*delta_time/np.pi # TODO: replace with your code to compute the baseline parameter using scale, wheel_vel, and delta_time
    baseline /= num
